...
public class praktikum7 {
    public static void main(String[] args) {
        HashMap<String, ArrayList<String>> filmfav = new HashMap<>();
        Scanner scan = new Scanner(System.in);
        filmfav.put("Judul", new ArrayList<>());
        filmfav.put("Rilis", new ArrayList<>());
        filmfav.put("Durasi", new ArrayList<>());
        filmfav.put("Genre", new ArrayList<>());
        filmfav.put("Sinopsis", new ArrayList<>());
        filmfav.put("Cast", new ArrayList<>());
        String lala = "Y";
        while(lala.equalsIgnoreCase("Y")){
            daftfilm(filmfav);
            System.out.println("(d) detail (s) search (a) add (r) remove");
            String input = scan.next();
        if(input.equalsIgnoreCase("d")){
            int noFilm = scan.nextInt() - 1;
            detailFilm(noFilm,filmfav);
            System.out.println("Y/N untuk melanjutkan daftar Film");
            lala = scan.next();
        }
        else if(input.equalsIgnoreCase("a")){
            addFilm(filmfav,scan);
            daftfilm(filmfav);
            System.out.println("Y/N untuk melanjutkan daftar Film");
            lala = scan.next();
        }
            ...
            removeFilm(deleteNumberOfFilm,filmfav);
            daftfilm(filmfav);
            System.out.println("Y/N untuk melanjutkan daftar Film");
            lala = scan.next();
        }
            ...
            System.out.printf(search(input2,filmfav));
    ...
    public static void daftfilm(HashMap<String, ArrayList<String>> movieFavourite){
